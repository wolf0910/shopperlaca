...
if( count($_REQUEST['quantity']) != count($_REQUEST['id_product'])){
  echo json_encode( array('success'   => 'false','message' => 'array quantity and array id_product not match !!') );
   exit();
}


if( isset($_REQUEST['id_user']) && isset($_REQUEST['customer_name']) && isset($_REQUEST['customer_address']) && isset($_REQUEST['customer_phone']) && isset($_REQUEST['quantity']) && isset($_REQUEST['id_product']) )
{
  ...
  for( $i=0; $i<count($_REQUEST['id_product']); $i++ )
  {
    $sql_tmp='SELECT * FROM table_product WHERE id_product='.$_REQUEST['id_product'][$i].' ';
    ...
    $sql2=' INSERT INTO table_order_detail SET id_order = "'.$id_order.'", id_product = "'.$_REQUEST['id_product'][$i].'", quantity = '.$_REQUEST['quantity'][$i].', unit_price="'.$unit_price_tmp.'"  ';
